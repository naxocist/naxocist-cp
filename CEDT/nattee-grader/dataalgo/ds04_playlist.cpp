...
public:
  ...
  void reorder(int pos, std::vector<int> selected) {
    while(selected.size() && pos == selected.back()+1) {
      pos = selected.back();
      selected.pop_back();
    }
    ...
    for (int i = 0; i < mSize; ++i) {
      ...
      if (i == selected[c]) {
        ...
        pselected.push_back(crr);
        c++;
      }
    }

    node *nxt_pcrr = pcrr->next;

    pcrr->next = pselected.front();
    pselected.front()->prev = pcrr;

    for (int i = 0; i < (int)pselected.size() - 1; ++i) {
      pselected[i]->next = pselected[i + 1];
      pselected[i + 1]->prev = pselected[i];
    }

    pselected.back()->next = nxt_pcrr;
    nxt_pcrr->prev = pselected.back();
  }
};
} // namespace CP
