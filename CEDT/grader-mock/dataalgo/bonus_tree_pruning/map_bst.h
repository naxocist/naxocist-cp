...
public:
  ...
  //------- Additional functions for Tree Modify -------

  std::vector<KeyT> getValuesInorder() {
    std::vector<KeyT> v;
    getValuesInorder(mRoot, v);
    return v;
  }
  std::vector<KeyT> getValuesInorder(node *n, std::vector<KeyT> &v) {
    if (n == nullptr)
      return v;
    getValuesInorder(n->left, v);
    v.push_back(n->data.first);
    getValuesInorder(n->right, v);
    return v;
  }

  std::vector<KeyT> getValuesPreorder() {
    std::vector<KeyT> v;
    getValuesPreorder(mRoot, v);
    return v;
  }
  std::vector<KeyT> getValuesPreorder(node *n, std::vector<KeyT> &v) {
    if (n == nullptr)
      return v;
    v.push_back(n->data.first);
    getValuesPreorder(n->left, v);
    getValuesPreorder(n->right, v);
    return v;
  }
  ...
  void printResult() {
    ...
    std::cout << "\nInorder: ";
    for (auto x : getValuesInorder())
      std::cout << x << ' ';
    std::cout << "\nPreorder: ";
    for (auto x : getValuesPreorder())
      std::cout << x << ' ';
    std::cout << "\n";
  }
  ...
  void modify(map_bst &other);
  void graft(node *n, node *m, size_t otherSize);
  int prune(node *n, KeyT upper, KeyT lower, int aux1, int aux2);
};
